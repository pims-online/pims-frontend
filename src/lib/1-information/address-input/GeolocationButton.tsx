...
import { getUserLocation, getGeoplateformeFeaturesFromCoordinates } from "./utils";
import type { GeoplateformeApiFeature, HandlerWrapper } from "./types";

type Props = {
  setAddressFeatureList: Dispatch<SetStateAction<Array<GeoplateformeApiFeature>>>,
  handlerWrapper: HandlerWrapper;
}

export default function GeolocationButton(props: Props) {
  const { t } = useTranslation('information_screen');
  const { setAddressFeatureList, handlerWrapper
  } = props;
  ...
  const handleGetGeoplateformeFeature = async (latitude: number, longitude: number) => {
    // 1 - Update address Feature List
    const featureList = await getGeoplateformeFeaturesFromCoordinates({ latitude, longitude });
  ...
  return (
    <Container className="fr-my-4v" withoutMarginBottom flexboxAlignment="center" flexboxDirection="row">
      ...
      {isProcessing && (
        <CircularProgress color="blue" size="small" />
      )}
    </Container>
  )
}